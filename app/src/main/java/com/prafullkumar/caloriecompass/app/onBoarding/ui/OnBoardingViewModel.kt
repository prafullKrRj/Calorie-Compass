package com.prafullkumar.caloriecompass.app.onBoarding.ui
...
class OnBoardingViewModel(
    ...
    fun onNextClicked() {
        viewModelScope.launch {
            repository.getAddUserData(
                userDataEntity = UserDataEntity(
                    userName = uiState.name,
                    userWeight = uiState.weight.toInt(),
                    userHeight = uiState.height.toInt(),
                    userWeightingUnit = uiState.weighingUnit.name,
                    userGoal = uiState.goal.name,
                    userGender = uiState.gender.name,
                    userAge = uiState.age.toInt(),
                    userActivityLevel = uiState.activityLevel.name
                )
            )
        }
    }
